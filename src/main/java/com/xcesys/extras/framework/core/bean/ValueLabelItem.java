...
public class ValueLabelItem implements Comparable<ValueLabelItem>, Serializable {
	...
	public ValueLabelItem(Long id, String value, String label, String description) {
		super();
		this.id = id;
		this.value = value;
		this.label = label;
	...
	public ValueLabelItem(Long id, String value, String label, String description, String category) {
		super();
		this.id = id;
		this.value = value;
		this.label = label;
	...
	public int compareTo(ValueLabelItem item) {
		...
		if (ret == 0) {
			ret = (this.getValue() + this.getLabel()).compareToIgnoreCase(item.getValue() + item.getLabel());
		}
		return ret;
	}
		...
		if (displayLabel == null) {
			if (this.value != null && this.value.equals(this.label)) {
				...
				displayLabel = this.value
						+ ((this.label == null || this.label.trim().length() == 0) ? "" : (" - " + this.label));
			}
		}
		return displayLabel;
	}
	...
	public String toString() {
		final String TAB = "    ";
		String retValue = "";
		retValue = "ValueLabelItem ( " + super.toString() + TAB + "id = " + this.id + TAB + "code = " + this.value + TAB
				+ "name = " + this.label + TAB + "description = " + this.desc + TAB + "seqNum = " + this.seq + TAB
				+ " )";
